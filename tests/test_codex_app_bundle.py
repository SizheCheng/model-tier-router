...
from mtr_dogfood.codex_app_bundle import (
    HOOK_EVENTS,
...
class CodexAppBundleTests(unittest.TestCase):
    def test_zipapp_is_reproducible(self):
        files = {
            ...
            "mtr_dogfood/codex_app_enforcement.py": b"VALUE = 1\n",
    ...
    def test_requirements_enable_managed_hooks_without_disabling_user_hooks(self):
        text = render_requirements(
            r"C:\ProgramData\OpenAI\Codex\managed-hooks\mtr-dogfood-r1",
            r"C:\Users\fixture\.codex\mtr-dogfood-data",
        )
        ...
        for name, _matcher, _status in HOOK_EVENTS:
            ...
            self.assertIn("mtr-dogfood-codex-hook.pyz", command)
    ...
    def test_bundle_uses_clean_committed_sources_and_verifies_hashes(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            dogfood = _repository(
                root / "dogfood",
                {
                    "src/mtr_dogfood/codex_app_enforcement.py": (
                        b"def main(): return 0\n"
                    )
                },
            )
            ...
            artifact = output / "mtr-dogfood-codex-hook.pyz"
