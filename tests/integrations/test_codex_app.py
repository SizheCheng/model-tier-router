...
from model_tier_router.codex_app import (
...
class CodexAppEnforcementTests(unittest.TestCase):
    def test_development_prompt_calls_router_and_redacts_secrets(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            event = _event(
                "UserPromptSubmit",
                root,
                prompt=f"Implement the component with {credential_label}={canary}",
            )
            with mock.patch(
                "model_tier_router.codex_app._router_assessment",
    ...
    def test_canonical_redaction_marker_is_idempotent(self):
        credential_label = "api" + "_key"
        expected = f"{credential_label}=[REDACTED]"
        for value in (
            expected,
            ...
            with self.subTest(value=value):
                redacted, _changed = _redact_text(value)
                self.assertEqual(redacted, expected)
                redacted_again, changed_again = _redact_text(redacted)
                self.assertEqual(redacted_again, expected)
