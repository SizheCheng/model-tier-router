...
class CampaignLedger:
    ...
    def reserve(self, lane_id: str) -> dict[str, Any]:
        ...
        record = {
            ...
            "inherited_r5_ordinal": len(self.records) + 2,
...
def _validate_manifest(
    ...
    *,
    ...
    if (
        ...
        or manifest.get("maximum_new_starts") != 2
    ...
    if (
        ...
        or embedded_release.get("entrypoint") != "final-execution"
    ...
    expected_paths = {
        LANE_ORDER[0]: router_repository,
        LANE_ORDER[1]: qwen_repository,
    }
    for lane in lanes:
...
def _run_lanes(
    *,
    ...
    budget = ProcessAccounting(maximum=2)
    ...
    contract = {
        ...
        "repositories": {
            ...
            "qwen-redaction-standalone": {
                "path": str(execution_repositories[LANE_ORDER[1]]),
                "baseline_head": manifest["lanes"][1]["source_head"],
    ...
    for binding in manifest["lanes"]:
        ...
        actual = assess_live(
            actual_repositories[LANE_ORDER[0]],
        ...
        descriptor = {
            "branch_prefix": f"mtr-dogfood/final-two/{lane_id}",
    ...
    *,
    ...
    if qualification_only:
        with tempfile.TemporaryDirectory(
            prefix="final-two-qualification-",
            dir=workspace,
        ) as temporary:
            clone_root = Path(temporary)
            execution_repositories = {
                lane_id: _clone_repository(
                    actual_repositories[lane_id],
                    clone_root / f"repo-{index}",
                    manifest["lanes"][index - 1]["source_head"],
                )
