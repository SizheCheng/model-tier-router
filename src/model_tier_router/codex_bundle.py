...
RUNTIME_NAME = "model-tier-router-codex-hook.pyz"
REQUIREMENTS_NAME = "requirements.toml"
MANIFEST_NAME = "codex-app-data-collection-manifest.json"
HOOK_EVENTS = (
...
def deterministic_zipapp(files: dict[str, bytes], output: str | Path) -> dict[str, Any]:
    required = {
        "__main__.py",
        "model_tier_router/__init__.py",
        "model_tier_router/codex_app.py",
    }
...
def render_requirements(install_root: str | Path, data_root: str | Path) -> str:
    ...
    lines = [
        "# Managed model-tier-router development-data collection for Codex local clients.",
...
def build_managed_bundle(
    ...
    *,
    ...
    source_state = _repository_state(source)
    files = _router_files(source)
    files.update(
        {
            "__main__.py": (
                b"from model_tier_router.codex_app import main\n"
